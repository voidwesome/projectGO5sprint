...
func (t Training) TrainingInfo() InfoMessage {
	...
	return InfoMessage{
		...
		Calories:     t.Calories(),   // количество потраченных килокалорий на тренировке
	}
}
...
func (w Walking) Calories() float64 {
	...
	poweredMeanSpeedInMs := math.Pow((w.meanSpeed() * KmHInMsec), 2) // средняя скорость в метрах в секунду во 2 степени
	return ((CaloriesWeightMultiplier*w.Weight + (poweredMeanSpeedInMs/(w.Height/CmInM))*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours)
}
